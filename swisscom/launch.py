...
from swisscom.swisscom_ai.research_keyphrase.embeddings.emb_distrib_local import EmbeddingDistributorLocal
from swisscom.swisscom_ai.research_keyphrase.model.input_representation import InputTextObj
from swisscom.swisscom_ai.research_keyphrase.model.method import MMRPhrase
from swisscom.swisscom_ai.research_keyphrase.preprocessing.postagging import PosTaggingCoreNLP
...
def load_local_embedding_distributor(model_name='roberta-large-nli-stsb-mean-tokens', model_path=None):
    if model_name == 'sent2vec' or model_name == 'fasttext':
        current_dir = str(os.path.dirname(os.path.realpath(__file__)))
        config_parser = ConfigParser()
        config_parser.read("{}/config.ini".format(current_dir))
        return EmbeddingDistributorLocal(model_path, is_static=True)
